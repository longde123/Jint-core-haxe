...
namespace Jint.Native.Json
{
    public class JsonSerializer
    {
        ...
        public JsValue Serialize(JsValue value, JsValue replacer, JsValue space)
        {
            ...
            if (value.Is<ICallable>() && replacer.Equals( Undefined.Instance) )
            {
        ...
        private JsValue Str(string key, ObjectInstance holder)
        {
            ...
            if (value.IsObject())
            {
                ...
                if (toJson.IsObject())
                {
                    var callableToJson = (ICallable)toJson.AsObject();
            ...
            if (!_replacerFunction.Equals( Undefined.Instance))
            {
            ...
            foreach (char c in value)
            {
                switch (c)
                {
                    ...
                    default:
                        ...
                        {
                            product += "\\u";
                            product += TypeConverter.ToString((int) c,"x4");
                        }
                        else
                            product += c;
                        break;
        ...
        private string SerializeArray(ArrayInstance value)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                if (strP.Equals( JsValue.Undefined))
                    strP = "null";
        ...
        private void EnsureNonCyclicity(object value)
        {
            ...
            {
                throw new JavaScriptException().Creator(_engine.TypeError, "Cyclic reference detected.");
            }
        }
