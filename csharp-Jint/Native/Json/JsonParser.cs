    ...
    public class JsonParser
    {
        ...
        private char ScanHexEscape(char prefix)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                {
                    throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format("Expected hexadecimal digit:{0}", _source));
                }
            }
            return (char)code;
        }
        ...
        private Token ScanPunctuator()
        {
            ...
            switch ((int)code)
            {
                ...
                case 126: // ~
                    ...
                    token.Value = code.ToString();
                    token.LineNumber = _lineNumber;
                    token.LineStart = _lineStart;
                    token.Range = new[] { start, _index };

                    return token;
            }
            throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format(Messages.UnexpectedToken, code));
        }
        ...
        private Token ScanNumericLiteral()
        {
            ...
            token.Value = Double.Parse(number, NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent, CultureInfo.InvariantCulture);
            token.LineNumber = _lineNumber;
            token.LineStart = _lineStart;
            token.Range = new[] { start, _index };
        ...
        private Token ScanBooleanLiteral()
        {
            ...
            {
                throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format(Messages.UnexpectedToken, s));
            }
        }
        ...
        private Token ScanStringLiteral()
        {
            ...
            while (_index < _length)
            {
                ...
                if (ch <= 31)
                {
                    throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format("Invalid character '{0}', position:{1}, string:{2}", ch, _index, _source));
                }
            ...
            if (quote != 0)
            {
                throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format(Messages.UnexpectedToken, _source));
            }
        ...
        private void ThrowUnexpected(Token token)
        {
            ...
            ThrowError(token, Messages.UnexpectedToken, (string)token.Value);
        }
        ...
        public ObjectInstance ParseJsonObject()
        {
            ...
            while (!Match("}"))
            {
                ...
                {
                    throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format("Invalid character in property name '{0}'", name));
                }
        ...
        private JsValue ParseJsonValue()
        {
            ...
            switch (type)
            {
                ...
                case Tokens.BooleanLiteral:
                    return new JsValue().Creator((bool)Lex().Value);
                case Tokens.String:
                    return new JsValue().Creator((string)Lex().Value);
                case Tokens.Number:
                    return new JsValue().Creator((double)Lex().Value);
            }
        ...
        public JsValue Parse(string code, ParserOptions options)
        {
            ...
            _state.MarkerStack = new Stack<int>();


            _extra = new Extra();
            _extra.Range = new int[0];
            _extra.Loc = 0;
            ...
            {
                throw new JavaScriptException().Creator(_engine.SyntaxError, string.Format("Unexpected {0} {1}", _lookahead.Type, _lookahead.Value));
            }
            return jsv;
