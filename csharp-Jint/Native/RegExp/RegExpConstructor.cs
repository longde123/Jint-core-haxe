...
namespace Jint.Native.RegExp
{
    public sealed class RegExpConstructor : FunctionInstance, IConstructor
    {
        ...
        public override JsValue Call(JsValue thisObject, JsValue[] arguments)
        {
            ...
            if (!pattern.Equals( Undefined.Instance) && flags.Equals(Undefined.Instance) && TypeConverter.ToObject(Engine, pattern).Class == "Regex")
            {
                return pattern;
            }
        ...
        public ObjectInstance Construct(JsValue[] arguments)
        {
            ...
            if (flags.Equals(Undefined.Instance) && r != null)
            {
                return r;
            }
            else if (!flags.Equals(Undefined.Instance )&& r != null)
            {
                throw new JavaScriptException().Creator(Engine.TypeError);
            }
            else
            {
                if (pattern.Equals(Undefined.Instance))
                {
                    p = "";
                ...
                f = !flags.Equals(Undefined.Instance) ? TypeConverter.ToString(flags) : "";
            }
            ...
            {
                throw new JavaScriptException().Creator(Engine.SyntaxError, e.Message);
            }
        ...
        public RegExpInstance Construct(string regExp)
        {
            ...
            if (regExp[0] != '/')
            {
                throw new JavaScriptException().Creator(Engine.SyntaxError, "Regexp should start with slash");
            }
