...
namespace Jint.Native.RegExp
{
    public sealed class RegExpPrototype : RegExpInstance
    {
        ...
        private JsValue Test(JsValue thisObj, JsValue[] arguments)
        {
            ...
            return !match.Equals(Null.Instance);
        }

        internal JsValue Exec(JsValue thisObj, JsValue[] arguments)
        {
            var R = (RegExpInstance)TypeConverter.ToObject(Engine, thisObj);
            if (R == null)
            {
                throw new JavaScriptException().Creator(Engine.TypeError);
            }
            ...
            for (var k = 0; k < n; k++)
            {
                ...
                a.DefineOwnProperty(k.ToString(), new PropertyDescriptor().Creator(value, true, true, true), true);            
            }
        ...
        {
            array.DefineOwnProperty("index", new PropertyDescriptor().Creator(indexValue, writable: true, enumerable: true, configurable: true), true);
            array.DefineOwnProperty("input", new PropertyDescriptor().Creator(inputValue, writable: true, enumerable: true, configurable: true), true);
            array.DefineOwnProperty("length", new PropertyDescriptor().Creator(value: lengthValue, writable: false, enumerable: false, configurable: false), true);
            return array;
        }
    }
}
