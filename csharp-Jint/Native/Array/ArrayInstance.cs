...
namespace Jint.Native.Array
{
    public class ArrayInstance : ObjectInstance
    {
        ...
        private IDictionary<uint, PropertyDescriptor> _array = new MruPropertyCache<uint, PropertyDescriptor>();
        ...
        public override void Put(string propertyName, JsValue value, bool throwOnError)
        {
            ...
            {
                var valueDesc = new PropertyDescriptor().Creator(value: value, writable: null, enumerable: null, configurable: null);
            ...
            {
                var newDesc = new PropertyDescriptor().Creator(value, true, true, true);
        ...
        public override bool DefineOwnProperty(string propertyName, PropertyDescriptor desc, bool throwOnError)
        {
            ...
            if (propertyName == "length")
            {
                ...
                var newLenDesc = new PropertyDescriptor().Creator(desc);
                uint newLen = TypeConverter.ToUint32(desc.Value);
                if (newLen != TypeConverter.ToNumber(desc.Value))
                {
                    throw new JavaScriptException().Creator(_engine.RangeError);
                }
                ...
                if (_array.Count < oldLen - newLen)
                {
                    ...
                    foreach (var key in keys)
                    {
                        ...
                        if (IsArrayIndex(key, out keyIndex) && keyIndex >= newLen && keyIndex < oldLen)
                        {
                            ...
                            if (!deleteSucceeded)
                            {
                                newLenDesc.Value = new JsValue().Creator(keyIndex + 1);
                                if (!newWritable)
                                {
                ...
                if (!newWritable)
                {
                    DefineOwnProperty("length", new PropertyDescriptor().Creator(value: null, writable: false, enumerable: null, configurable: null), false);
                }
                return true;
            }
