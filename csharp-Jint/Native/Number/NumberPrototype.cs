...
namespace Jint.Native.Number
{
    ...
    public sealed class NumberPrototype : NumberInstance
    {
        ...
        private JsValue ToLocaleString(JsValue thisObject, JsValue[] arguments)
        {
            ...
            return TypeConverter.ToString(m,"n", Engine.Options.GetCulture());
        }
        ...
        private JsValue ToFixed(JsValue thisObj, JsValue[] arguments)
        {
            ...
            if (f < 0 || f > 20)
            {
                throw new JavaScriptException().Creator(Engine.RangeError, "fractionDigits argument must be between 0 and 20");
            }
            ...
            return TypeConverter.ToString(x, "f" + f, CultureInfo.InvariantCulture);
        }
        ...
        private JsValue ToExponential(JsValue thisObj, JsValue[] arguments)
        {
            ...
            return TypeConverter.ToString(x,format, CultureInfo.InvariantCulture);
        }
        ...
        private JsValue ToPrecision(JsValue thisObj, JsValue[] arguments)
        {
            ...
            if (arguments.At(0).Equals( Undefined.Instance))
            {
            ...
            if (p < 1 || p > 21)
            {
                throw new JavaScriptException().Creator(Engine.RangeError, "precision must be between 1 and 21");
            }

            // Get the number of decimals
            string str =TypeConverter.ToString( x,"e23", CultureInfo.InvariantCulture);
            ...
            return TypeConverter.ToString(x,"f" + p, CultureInfo.InvariantCulture);
        }
        ...
        private JsValue ToNumberString(JsValue thisObject, JsValue[] arguments)
        {
            ...
            var radix = arguments.At(0).Equals( JsValue.Undefined )? 10 : (int) TypeConverter.ToInteger(arguments.At(0));

            if (radix < 2 || radix > 36)
            {
                throw new JavaScriptException().Creator(Engine.RangeError, "radix must be between 2 and 36");
            }
        ...
        public static string ToNumberString(double m) 
        {
            ...
            string s = null;
            var rFormat = TypeConverter.ToString(m,"r");
            ...
            var parts =TypeConverter.ToString( m,format, CultureInfo.InvariantCulture).Split('e');
            if (s == null)
            {
