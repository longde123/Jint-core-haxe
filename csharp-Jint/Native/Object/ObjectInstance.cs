...
namespace Jint.Native.Object
{
    public class ObjectInstance
    {
        ...
        {
            Engine = engine;
            Properties = new MruPropertyCache<string, PropertyDescriptor>();
        }
        ...
        public JsValue DefaultValue(Types hint)
        {
            if (hint == Types.String || (hint == Types.None && Class == "Date"))
            {
                ...
                if (toString != null)
                {
                    var str = toString.Call(new JsValue().Creator(this), Arguments.Empty);
                    if (str.IsPrimitive())
                    {
                        return str;
                    }
                ...
                if (valueOf != null)
                {
                    var val = valueOf.Call(new JsValue().Creator(this), Arguments.Empty);
                    if (val.IsPrimitive())
                    {
                        return val;
                    }
        ...
        public virtual bool DefineOwnProperty(string propertyName, PropertyDescriptor desc, bool throwOnError)
        {
            ...
            // Step 6
            if (
                current.Configurable.Value == desc.Configurable.Value &&
        ...
        {
            SetOwnProperty(name, new PropertyDescriptor().Creator(value, writable, enumerable, configurable));
        }
