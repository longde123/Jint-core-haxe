...
namespace Jint.Runtime.Interop
{
    ...
    public sealed class ObjectWrapper : ObjectInstance, IObjectWrapper
    {
        ...
        public override PropertyDescriptor GetOwnProperty(string propertyName)
        {
            ...
            if (methods.Any())
            {
                var descriptor = new PropertyDescriptor().Creator(new MethodInfoFunctionInstance(Engine, methods), false, true, false);
                Properties.Add(propertyName, descriptor);
                return descriptor;
            }
            ...
            {
                return new IndexDescriptor(Engine, Target.GetType(), propertyName, Target);
            }
            ...
            // try to find explicit method implementations
             //todo 
            /*
            MethodInfo[] explicitMethods = (from iface in interfaces
                                   from imethod in iface.GetMethods()
                                   where EqualsIgnoreCasing(imethod.Name, propertyName)
                                   select imethod).ToArray();
               */
            MethodInfo[] explicitMethods = null;
            if (explicitMethods.Length > 0)
            {
                var descriptor = new PropertyDescriptor().Creator(new MethodInfoFunctionInstance(Engine, explicitMethods), false, true, false);
                Properties.Add(propertyName, descriptor);
                return descriptor;
            }
            ...
            var explicitIndexers =
                (from iface in interfaces
                 ...
                 select iprop).ToArray();
                   ...
                   */
            PropertyInfo[] explicitIndexers = null;
